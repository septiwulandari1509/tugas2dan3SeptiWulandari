...
class Students {
    ...
    static function selectWhere($clause) {
        global $conn;
        $sql = "SELECT s.id, s.name, s.email, r.role_name FROM students s, roles r WHERE s.role_fk = r.id " . $clause . ";";
        $result = $conn->query($sql);
        $arr = array();
    ...
    static function deleteWhere($clause) {
        global $conn;
        $sql = "DELETE FROM students s, roles r WHERE s.role_fk = r.id " . $clause . ";";
        $conn->query($sql);
        $result = $conn->affected_rows > 0 ? true : false;
        return $result;
    } # Ditugaskan!
